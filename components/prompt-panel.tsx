...
const PROMPT_TEMPLATES = [
  {
    label: "商超门店场景",
    value: `请帮我检测商超、便利店、收银区等零售场景中的异常事件，并补充完整类别定义后用于后续推理：
...
6. 人员跌倒：顾客或员工因滑倒、绊倒、失衡等原因突然倒地，需要与正常坐下、蹲下区分开。`,
  ...
  {
    label: "道路交通场景",
    value: `请帮我检测道路、路口、停车场或高架桥监控中的异常事件，并补充完整类别定义后用于后续推理：
1. 交通事故：车辆碰撞、追尾、侧翻、失控冲出道路或撞击护栏、行人、非机动车。
2. 车辆起火：车辆出现持续明火、冒烟、燃烧或局部火势蔓延。
3. 车辆爆炸：车辆或路面发生瞬时爆燃、强冲击、碎片飞散或剧烈烟尘扩散。
...
6. 道路拥堵或异常聚集：车辆大面积滞留、异常排队，或事故后人群在道路中央聚集。`,
  ...
  {
    label: "社区园区场景",
    value: `请帮我检测小区、园区、楼宇出入口、广场等公共监控场景中的异常事件，并补充完整类别定义后用于后续推理：
...
6. 明火或烟雾：公共区域出现持续火焰、浓烟、燃烧物或明显起火迹象。`,
  ...
  {
    label: "工地厂区场景",
    value: `请帮我检测工地、仓库、厂房、生产车间等场景中的异常事件，并补充完整类别定义后用于后续推理：
...
6. 聚集性冲突或追逐：多人出现明显争执、推搡、追赶或暴力冲突。`,
  ...
  {
    label: "校园医院场景",
    value: `请帮我检测校园、医院、办公楼大厅、电梯厅等室内公共场景中的异常事件，并补充完整类别定义后用于后续推理：
...
6. 烟火异常：走廊、大厅、电梯口等区域出现可见明火、浓烟或燃烧痕迹。`,
  ...
  {
    label: "水域灾害场景",
    value: `请帮我检测河道、海边、港口、堤坝、景区或灾害监控场景中的异常事件，并补充完整类别定义后用于后续推理：
